...
- A cada passo, $V$, $E$, $F$ (e conchas, laços, alças) variam de modo a **preservar a fórmula de Euler-Poincaré**
...
:::
:::col width=48%
- Forma generalizada (conchas $S$, laços internos $L$, alças/_genus_ $G$):
$$
\small
V - E + F - L = 2\,(S - G)
$$
- Exemplos (cada um mantém o invariante):
  - **MVFS** (_Make Vertex-Face-Shell_): cria a malha inicial — $\Delta V=\Delta F=\Delta S=1$
  - **MEV** (_Make Edge-Vertex_): $\Delta V=\Delta E=1$
  - **MEF** (_Make Edge-Face_): $\Delta E=\Delta F=1$ (divide uma face)
  - Operadores próprios tratam **laços** (furos em faces) e **alças** (_genus_)
:::
---
...
---
# Rendering com WebGL
- Cada vértice carrega **atributos** além da posição: **cor**, **normal**, **coordenadas de textura**
- É preferível usar **`drawElements`** em vez de `drawArrays`:
  - Cada vértice pode ser **reutilizado** por várias faces
  - `drawElements` usa os **índices** dos vértices (a própria tabela de faces!)
  - `drawArrays` precisaria **repetir** as coordenadas do vértice a cada face
- A estrutura de malha (tabela de vértices + índices) mapeia diretamente para os _buffers_ da GPU
- Exemplo: [observablehq.com/@esperanc/webgl-cube](https://observablehq.com/@esperanc/webgl-cube)
---
# Resumo
